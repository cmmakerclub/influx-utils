...
EOF
  ...
  if confirm ;then
    ...
    docker run \
          ...
          unset INFLUX_TELEGRAF_PASSWORD
          while [ -z ${INFLUX_TELEGRAF_PASSWORD} ]; do
               read -r -p "Enter INFLUX_TELEGRAF_PASSWORD: " INFLUX_TELEGRAF_PASSWORD
          done

          echo "INFLUX_TELEGRAF_USER=$INFLUX_TELEGRAF_USER"
          echo "INFLUX_TELEGRAF_PASSWORD=$INFLUX_TELEGRAF_PASSWORD"

          influx -execute "CREATE USER \"${INFLUX_ADMIN_USER}\" WITH PASSWORD '${INFLUX_ADMIN_PASSWORD}' WITH ALL PRIVILEGES"
          influx -execute "CREATE USER \"${INFLUX_TELEGRAF_ADMIN}\" WITH PASSWORD '${INFLUX_TELEGRAF_PASSWORD}' WITH ALL PRIVILEGES" 
  fi
}
